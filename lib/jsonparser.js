...
phssr.makeScriptTag = function(){
    ...
    scrpTag +="var o = e.srcElement || e.target;";
    scrpTag +="o.parentElement.getElementsByClassName('traceinfo')[0].className = 'traceinfo visible';}";
    scrpTag +="function closeTraceModal(e){";
    scrpTag +="var o = e.srcElement || e.target;";
    scrpTag +="o.parentElement.parentElement.className = 'traceinfo';}";
...
phssr.makeHardCodedStyleTag = function(reporterOptions){
    ...
    styleTag+= "td.duration-col{width:100px;}th.duration-col{width: 100px;}";
    styleTag+= "td.browser-col{width:345px;}th.browser-col{width: 345px;}";
    //styleTag+= "td.sessionId-col{width:100px;}th.sessionId-col{width: 100px;}";
...
phssr.makeHTMLPage = function(tableHtml, reporterOptions){
    ...
    staticHTMLContentprefix +=  "<h1></h1><table class='header'>";
    staticHTMLContentprefix +=  "<tr><th class='desc-col'>Description</th><th class='status-col'>Passed</th><th class='duration-col'>Duration (ms)</th>";
    staticHTMLContentprefix +=  "<th class='browser-col'>Browser</th>";
    //staticHTMLContentprefix +=  "<th class='sessionId-col'>Session_ID</th>";
...
function generateHTML(data){
    ...
    var str = '<table class="content"><tr>';
    ...
    str +=  '<td class="duration-col">' + data.duration + '</td>';
    str +=  '<td class="browser-col">' + data.browser.name+ ':' +data.browser.version + '</td>';
    //str +=  '<td class="sessionId-col">' + data.sessionId + '</td>';
    ...
    if(!(reporterOptions.takeScreenShotsOnlyForFailedSpecs && data.passed)) {
        var imgPath = path.basename(data.screenShotFile);
        str +=  '<td class="img-col"><a onclick="openModal(\'' + imgPath + '\')"><img src=" '+ imgPath +' " style="max-height:100px"/> </a></td>';
    }
    else{
...
function processJson(jsonData, options){
    ...
    result = "<div><h2><u>Results summary</u></h2>";
    result += "<b>Total tests passed</b>: "+ passCount +" <br/> <b>Total tests failed</b>: "+ failCount +" <br/> This report generated on "+new Date()+" </div>";
